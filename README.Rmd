...
```
Look at Rhat stats for `msiAssess` model run; not available for `BRCIndicators`: see issue [here](https://github.com/BiologicalRecordsCentre/BRCindicators/issues/90). It's worth noting here that the global SE parameter, `theta`, is far from converging. The same behaviour has been observed with an empirical dataset with longer chains (`n_iter = 1000`).
